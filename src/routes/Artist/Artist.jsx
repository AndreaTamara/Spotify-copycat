...
export const Artist = () => {
    ...
    return (
        <DetailViewContainer>
            {artistLoading &&
                <div className='hero-artist-img'>
                    <Loader height='12rem' />
                </div>}
            {artist &&
                <img className='hero-artist-img'
                    src={selectImage(artist.images[0]?.url, 'artistCover')}
                    alt='artist' />}
            {artistError ?
                <Info
                    msn={`Error ${topTracksError?.status}: ${topTracksError?.message}`} />
                :
                <DetailHeader
            ...
            <RowList
                title='Discography'
                id={'artist-discography' + artistId}
                artistView={true}
            >
                {albumsArtistLoading && <Loader />}
                {albumsArtistError &&
                    <Info msn={`Error ${albumsArtistError?.status}: ${albumsArtistError?.message}`} />}
            ...
            <RowList
                ...
                id={'related-artist' + artistId}
                artistView={true}
            >
                {artistsRelatedLoading && <Loader />}
                {artistsRelatedError &&
                    <Info msn={`Error ${artistsRelatedError?.status}: ${artistsRelatedError?.message}`} />}
